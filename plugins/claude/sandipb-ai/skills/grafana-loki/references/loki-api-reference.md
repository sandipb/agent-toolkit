...
```
Params: `query` (stream selector), `start`, `end`. Relative `since` is not supported.
...
```
Params: `query`, `start`, `end`, `since`, `limit` (default 1000; `field_limit` is a backward-compatible alias),
`line_limit` (default 100), `step`
...
```
Uses the same time and limit parameters as detected fields.
...
POST /otlp/v1/logs
```
When configuring an OpenTelemetry Collector `otlphttp` exporter, use an endpoint ending in `/otlp`; the exporter
appends `/v1/logs`.
