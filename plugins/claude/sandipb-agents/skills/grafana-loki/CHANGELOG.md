...
### Changed
- Expanded activation guidance for Loki terminology and common query tasks.
- Made bundled paths portable across Codex, ChatGPT, and Claude Code installs.
- Clarified optional tenant IDs, current label guidance, and configurable query limits.
- Added current Alloy and OTLP ingestion guidance and detected-field API details.
- Documented reliable shell environment patterns and nested JSON query handling.
- Hardened the HTTP helper with argument validation and explicit write guards.

### Fixed
- Rejected unsupported `--since` usage for the index stats API.
- Removed the non-functional plain-HTTP fallback for the WebSocket tail endpoint.
- Corrected detected-fields parameter names and defaults.
