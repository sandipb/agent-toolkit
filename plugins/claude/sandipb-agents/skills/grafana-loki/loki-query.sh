...
#   LOKI_ALLOW_WRITE   - Set to "true" only after approval for push/non-GET raw calls
#
# Commands:
...
_require_write_enabled() {
    if [[ "${LOKI_ALLOW_WRITE:-}" != "true" ]]; then
        echo "Error: external writes are disabled; obtain approval, then set LOKI_ALLOW_WRITE=true" >&2
        return 2
    fi
}
...
_parse_time_flags() {
    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            --since|--start|--end|--limit|--step|--interval|--direction|--time|--query|--targetLabels|--aggregateBy|--field_limit|--line_limit|--delay_for|--mode)
                _require_arg "value for $1" "${2:-}"
                local key="${1#--}"
                [[ "$key" == "query" ]] && _params+=("query=$(_urlencode "$2")") || _params+=("${key}=$2")
                shift 2
                ;;
    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            --start|--end)
                ...
                ;;
            --since)
                echo "Error: stats does not accept --since; provide --start and --end" >&2
                return 2
                ;;
            *) echo "Unknown stats flag: $1" >&2; return 2 ;;
        esac
    done
    local qs; qs=$(_build_qs "${params[@]}")
...
cmd_tail() {
    echo "Error: /tail requires WebSocket support; use logcli --tail or websocat" >&2
    return 2
}
...
cmd_raw() {
    ...
    local path="$1"; shift
    method="${method^^}"
    [[ "$method" == "GET" ]] || _require_write_enabled
...
Environment: LOKI_ADDR (required), LOKI_ORG_ID, LOKI_USERNAME, LOKI_PASSWORD,
             LOKI_BEARER_TOKEN, LOKI_CA_CERT, LOKI_TLS_SKIP, LOKI_ALLOW_WRITE
...
case "$COMMAND" in
    query_range)      _require_arg "LogQL query" "${1:-}"; cmd_query_range "$@" ;;
    query)            _require_arg "LogQL query" "${1:-}"; cmd_query "$@" ;;
    labels)           cmd_labels "$@" ;;
    label_values)     _require_arg "label name" "${1:-}"; cmd_label_values "$@" ;;
    series)           _require_arg "stream selector" "${1:-}"; cmd_series "$@" ;;
    stats)            _require_arg "stream selector" "${1:-}"; cmd_stats "$@" ;;
    volume)           _require_arg "stream selector" "${1:-}"; cmd_volume "$@" ;;
    volume_range)     _require_arg "stream selector" "${1:-}"; cmd_volume_range "$@" ;;
    detected_fields)  _require_arg "stream selector" "${1:-}"; cmd_detected_fields "$@" ;;
    push)             _require_arg "JSON body" "${1:-}"; cmd_push "$@" ;;
    ready)            cmd_ready ;;
    config)           cmd_config "$@" ;;
    buildinfo)        cmd_buildinfo ;;
    format)           _require_arg "LogQL query" "${1:-}"; cmd_format "$@" ;;
    tail)             cmd_tail "$@" ;;
    raw)              _require_arg "HTTP method" "${1:-}"; _require_arg "API path" "${2:-}"; cmd_raw "$@" ;;
    -h|--help|help)   usage ;;
