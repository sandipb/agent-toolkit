...
description: >-
  ...
  documents, API documentation, and substantive work prose. Prefer technical-writing for concise fragments such as
  short PR descriptions, commit messages, warnings, and code comments; not technical validation.
---
...
When composing or editing, apply safe language corrections inline. When editing, return additions of claims, distinct
information, or examples, removal of distinct information, major restructuring, changed emphasis, and unresolved
ambiguity as author-judgment suggestions.
...
accepted decisions, relevant sources, and applicable invariants. State the language-only scope and keep unrelated
context out of a new subagent's packet.
...
length, punctuation, and conversational tone are not independent acceptance criteria. Deduplicate findings. When
composing or editing, fix specific filler, formulaic phrasing, ornamental wording that adds no information or useful
emphasis, awkward rhythm, or unclear passages when the change preserves technical meaning. In review-only mode, return
findings and suggested fixes without changing the draft. Reject mechanical-only flags and changes that conflict with
the shared invariants. Reconcile findings in the main thread or with an editor while preserving the requested mode.
If the skill is unavailable, report that the pass was not run and continue the core workflow. Repeat the pass only after
material revisions introduce new prose.
